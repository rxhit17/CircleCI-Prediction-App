...
class DataProcessing:
    def __init__(self,file_path):
        ...
        self.df = None
        self.processed_data_path = 'artifact/processed'
    ...
    def handle_outliers(self, column):
        try :
            logger.info("staring handle outlier operation")
            ...
            selap_median = np.median(self.df[column])

            for i in self.df[column]:
                if i > upper_value or i < lower_value:
                    self.df[column] = self.df[column].replace(i,selap_median)
    ...
    def split_data(self):
        try:
            ...
            logger.info("file save sucessfull for data processing step...")

        except Exception as e :
            logger.error(f"error while saving the data processing  {e}")
            raise CustomException("fail to save the data  ", e )
